...
static llama_mmap::ranges ranges_complement(llama_mmap::ranges ranges, size_t limit) {
    llama_mmap::ranges result;
    std::sort(ranges.begin(), ranges.end());

    size_t pos = 0;
    for (const auto & range : ranges) {
        const size_t begin = std::min(range.first, limit);
        const size_t end   = std::min(range.second, limit);
        if (begin > pos) {
            result.emplace_back(pos, begin);
        }
        pos = std::max(pos, end);
    }
    if (pos < limit) {
        result.emplace_back(pos, limit);
    }
    return result;
}
#endif

llama_mmap::ranges llama_mmap::initial_prefetch_ranges(size_t file_size,
                                                       const llama_mmap::ranges & lazy_ranges,
                                                       size_t prefetch) {
    if (file_size == 0 || prefetch == 0) {
        return {};
    }
#if defined(_POSIX_MAPPED_FILES) || defined(_WIN32)
    return ranges_complement(lazy_ranges, std::min(file_size, prefetch));
#else
...
struct llama_mmap::impl {
    ...
    impl(struct llama_file * file, size_t prefetch, bool numa, const llama_mmap::ranges & lazy_ranges) {
        ...
        if (prefetch > 0) {
            for (const auto & range : llama_mmap::initial_prefetch_ranges(file->size(), lazy_ranges, prefetch)) {
                if (posix_madvise((char *) addr + range.first, range.second - range.first, POSIX_MADV_WILLNEED)) {
    ...
    impl(struct llama_file * file, size_t prefetch, bool numa, const llama_mmap::ranges & lazy_ranges) {
        ...
        if (prefetch > 0) {
            ...
            if (pPrefetchVirtualMemory) {
                ...
                for (const auto & range : llama_mmap::initial_prefetch_ranges(size, lazy_ranges, prefetch)) {
